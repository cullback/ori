...
use core_ir::Scalar::{Bool, I64, U8};
...
fn let_does_not_leak_outside_body() {
    ...
    let prog = b.call(plus, [inner_let, x.expr()], I64);
...
fn fold_targets_a_value() {
    ...
    let xs = b.local(list_i64);
    let sum_step = b.func();
    let prog = b.fold(sum_step, &xs, [b.int(0)], [], I64);
...
fn nested_fold_is_just_nested_value() {
    ...
    let outer = b.fold(walk, inner, [b.int(0)], [g.expr()], I64);
...
fn gen_makes_anamorphism_first_class() {
    ...
    let prog = b.gen_(b.int(10), range_step, [b.int(0)], [], I64, list_i64);
...
fn crash_is_a_leaf() {
    let b = Builder::new();
    validate_scope(&[], &b.crash("kaboom", I64)).expect("Crash carries no Var refs");
}
...
fn dag_check_catches_mutual_recursion() {
    ...
    let f = b.func();
    let g = b.func();
    let f_body = b.call(g, [], I64);
    let g_body = b.call(f, [], I64);
