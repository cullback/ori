...
fn app_renders_call_shape() {
    ...
    let plus = b.func();
    assert_eq!(
        format!("{}", b.call(plus, [b.int(1), b.int(2)], b.i64())),
        "@0(1:I64, 2:I64):I64",
    );
}
...
fn crash_renders_with_msg() {
    let b = Builder::new();
    assert_eq!(format!("{}", b.crash("kaboom", b.i64())), "Crash(\"kaboom\"):I64");
}
...
fn buf_lit_renders_compact() {
    let mut b = Builder::new();
    let list_u8 = b.list_of(b.u8());
    let prog = b.buf_lit([b.byte(b'h'), b.byte(b'i')], b.u8(), list_u8);
    let rendered = format!("{prog}");
