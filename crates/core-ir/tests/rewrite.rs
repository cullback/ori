...
fn free_vars_of_let_excludes_bound_var() {
    ...
    let x = b.local(I64);
    let y = b.local(I64);
    let prog = b.bind(&x, b.int(1), b.call(plus, [x.expr(), y.expr()], I64));
...
fn dead_let_preserved_when_value_crashes() {
    let mut b = Builder::new();
    let x = b.local(I64);
    let prog = b.bind(&x, b.crash("boom", I64), b.int(99));
    let after = simplify(prog, &FnTotality::new());
...
fn dead_let_with_total_app_eliminates() {
    ...
    let pure_fn = b.func();
    let x = b.local(I64);
    let prog = b.bind(&x, b.call(pure_fn, [b.int(1)], I64), b.int(99));
...
fn dead_let_with_unknown_callee_preserved() {
    ...
    let unknown = b.func();
    let x = b.local(I64);
    let prog = b.bind(&x, b.call(unknown, [b.int(1)], I64), b.int(99));
    let after = simplify(prog, &FnTotality::new());
    assert!(matches!(&after, Expr::Let { .. }));
}
