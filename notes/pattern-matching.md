...
# --- Record patterns ---
if event
    : Click{ x, y } then handle_click(x, y)       # bind fields by name
    : KeyPress{ key, .. } then handle_key(key)     # ignore extra fields
    : Custom{ name, ..rest } then forward(rest)    # capture remaining fields

# The : in record patterns introduces a sub-pattern for a field.
# Rename is the trivial case (bare binding); nested matching is the general case.
{ name: n, age: a } = user              # rename: bind n and a
{ name: _, age } = user                 # discard specific field
{ result: Ok(val), .. } = response      # nested pattern on a field
{ name, ..rest } = user                 # rest : { age: I32, email: Str }
...
An earlier design used `is` to start each arm. This creates a fundamental conflict: `is` is an infix expression operator (`x is Ok(val)` returns Bool), so using it as an arm delimiter means the same token serves two grammatical roles. Three specific problems make this unworkable:

**1. The expression grammar eats the first arm.** In `if x is Ok(val) then ...`, the expression parser greedily consumes `x is Ok(val)` as a boolean — it has no reason to stop before `is`. The multi-arm parser never gets a turn because `is` is already consumed. A PEG grammar confirms this: the `expr` rule matches `x is Ok(val)`, then `arm+` expects `is` but finds `then`.

**2. Arm results containing `is` are ambiguous.** In `if x is Ok(val) then f(val) is Valid is Err(e) then handle(e)`, the parser can't tell whether `is Valid` extends the arm result expression (`f(val) is Valid` → Bool) or starts a new arm matching `Valid`. Lookahead to `then` doesn't help — `then` also appears inside nested `if` expressions in arm results, so the parser would need to track `if`/`then` nesting depth. And since `is` is chainable (`a is B is C` is syntactically valid — testing whether a Bool matches a pattern), the expression grammar can always extend by consuming another `is`.

**3. Restricting arm results creates silent misparses.** The obvious fix — ban `is` from scrutinees and arm results — causes `if x is Foo(y) then f(y) is Ok(z) then z` to silently parse as two arms (`Foo(y) → f(y)`, `Ok(z) → z`) instead of one arm with result `f(y) is Ok(z)`. The grammar accepts it, produces a valid tree, but it means something completely different. No error, no warning.

Using `:` for arms eliminates all three problems. `:` is not an expression operator, so the expression grammar never consumes it. When the parser sees `:`, the previous expression is definitively over — zero lookahead, no restricted expression contexts, no silent misparses.
...
The tradeoff: `|` is more visually sparse and scans better at line starts in multiline patterns. But Ori already uses `then` instead of `->` and `and` for guards — it's a keyword-centric design where `or` fits naturally. In multiline or-patterns, `:` at line start always signals a new arm, so indented `or` continuations are unambiguous:
